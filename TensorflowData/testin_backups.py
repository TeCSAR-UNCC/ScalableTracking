...
class ObjectTracking:
    ...
    tracker_weights_file = 'model_mixed_dataset_finetune/model_step6_finetune.ckpt'  
    output_file = open("model_mixed_dataset_finetune/1LSTM_testing_results.txt", "a")
    #output_file1 = open("model_mixed_dataset/OT-successplot_Originalrolo.txt", "a")
    #prediction_file = open('model_mixed_dataset/prediction.txt','a')
...
#
    ...
    def testing(self, filefeatures, filebboxes,filegt,sequence_name,w_img, h_img):
        ...
        print('shape of LSTm out is ', predict.shape)
        self.predict_location = predict[-1][:, 255:259]
        print('shape of predict_location is ', self.predict_location.shape)
        ...
        with tf.Session() as sess:
            ...
            while id < self.testing_iters - self.num_steps:#*self.batch_size:
                ...
                batch_groundtruth = self.rolo_utils.load_rolo_gt(self.w_img,self.h_img,filegt, self.batch_size, self.num_steps, id)
                ...
                #print("Tracker Pred : ", (predict_location[0][0])*w_img, (predict_location[0][1])*h_img, (predict_location[0][2])*w_img, (predict_location[0][3])*h_img)
                #print("GT: ", (batch_groundtruth[0][0])*w_img, (batch_groundtruth[0][1])*h_img, (batch_groundtruth[0][2])*w_img, (batch_groundtruth[0][3])*h_img)
                #print('/n')
                ...
                if id % self.display_step == 0:
                    ...
                    #midlist=[(predict_location[0][0])*self.w_img,(predict_location[0][1])*self.h_img,(predict_location[0][2])*self.w_img,(predict_location[0][3])*self.h_img]
                    midlist=[(predict_location[0][0]-(predict_location[0][2]/2.0))*640.0,(predict_location[0][1]-(predict_location[0][3]/2.0))*480.0,((predict_location[0][2]/2.0)+predict_location[0][0])*640.0,((predict_location[0][3]/2.0)+predict_location[0][1])*480.0]
                    midlist1=[(predict_location[1][0]-(predict_location[1][2]/2.0))*640.0,(predict_location[1][1]-(predict_location[1][3]/2.0))*480.0,((predict_location[1][2]/2.0)+predict_location[1][0])*640.0,((predict_location[1][3]/2.0)+predict_location[1][1])*480.0]                    
 
 
                    predicted_location_list.append(midlist)
                    predicted_location_list1.append(midlist1)
            ...
            np.save('model_mixed_dataset_finetune/'+sequence_name+'prediction',predicted_array)    	    

            predicted_array1 = np.asarray(predicted_location_list1)
            np.save('model_mixed_dataset_finetune/'+sequence_name+'prediction1',predicted_array1)    	    
    ...
    def TrackerModel(self):
        ...
        num_videos = 1#modification
        for test in range(num_videos):
            test= test+59  #feeding 3 training videos itself
            # if test==38:
            #     continue
            [self.w_img, self.h_img, sequence_name, dummy_1, self.testing_iters] = utils.choose_video_sequence(test)
            x_path = os.path.join('own_images/testing/', sequence_name, 'features_video.npy') #Modification 4
            x_bbox_path = os.path.join('own_images/testing/', sequence_name, 'bbox_video.npy') #Modification 4

            y_path = os.path.join('own_images/testing/', sequence_name, 'gt.npy')    #Modification 5
            filegt =np.load(y_path)

            video_frame_f =np.load(x_path)
            video_frame_features = np.reshape(video_frame_f,newshape=(video_frame_f.shape[0]*video_frame_f.shape[1],video_frame_f.shape[2]))

            scaler = MinMaxScaler(feature_range=(-1,1))
            datanew = scaler.fit_transform(video_frame_features)
            datanew = np.reshape(datanew,newshape=(video_frame_f.shape[0],2,255))
            filefeatures = datanew      
            filebboxes = np.load(x_bbox_path)
            #self.output_path = os.path.join('MOT_data/train', sequence_name, 'rolo_out_Testing_1LSTM/')
            self.output_path = os.path.join('mixed_dataset/Testing', sequence_name, 'rolo_out_test_1LSTM/')
