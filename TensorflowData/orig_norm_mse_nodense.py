...
class ObjectTracking:
    rolo_weights_file = 'model_mixed_dataset/model_step6.ckpt' #Modification 1 
    rolo_out_finetune='model_mixed_dataset_finetune/model_step6_finetune.ckpt'
...
#
    def BuildLSTM(self, name,  input_x, initstate, weights, biases,reuse):
        with tf.variable_scope("LSTM") as scope:
            ...
            output = tf.transpose(output, [1, 0, 2]) #Comment if want ot keep 1,6,259 (like mayuri)
            reuse = True
            tf.get_variable_scope().reuse_variables()
    ...
    def training(self):
        ...
        log_file = open("model_mixed_dataset_finetune/OT-originalLSTM_training-log.txt", "a") #open in append mode
        self.build_LSTM_networks()
        num_videos = 6  #Trained on 15 of these technically
        # count=0.0
        # loss_list=[]
        epoches = 6 * 20
        ...
        self.predict_location = predict[-1][:,255:] #Picking the last frame --> change this to 0 and comment transpose above to make it work like mayuri
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epoches):
                #i = epoch % num_videos
                i = epoch % num_videos
                i=i+17 #in ROLO utils file, the sequence for MOT16 starts with 30 Modification 3
                # if i==3 or i==4 or i==6 or i==7 or i==9 or i==12 or i==21 or i==22 or i==23 or i==24:
                #     continue
                [self.w_img, self.h_img, sequence_name, self.training_iters, dummy]= utils.choose_video_sequence(i)
        
                x_path = os.path.join('own_images/training/', sequence_name, 'features_video.npy') #Modification 4
                x_bbox_path = os.path.join('own_images/training/', sequence_name, 'bbox_video.npy')

                y_path = os.path.join('own_images/training/', sequence_name, 'gt.npy') ##Modification 5
                #print y_path
                ...
                self.output_path = os.path.join('own_images/training', sequence_name, 'rolo_out_train_1LSTM/')
                ...
                save_path = self.saver.save(sess, self.rolo_out_finetune)
